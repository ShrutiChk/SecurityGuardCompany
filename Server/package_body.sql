...
create or replace package body myPackage as
	...
	BEGIN
		...
		select sum(g_sal) into t_sal from guards;
		t_sal := t_sal*12;
	...
	BEGIN
		...
		select Amount into h_income from Payment_monthly_info where payid = n;
		select Amount into s_income from Payment_monthly_info@site_link where payid = n;
		m_income := h_income + s_income;
		
		if(t_sal > m_income)then
			n2 := 0;
			total := t_sal - m_income;
			--return loss;
		elsif(t_sal < m_income) then
			n2 := 1;
			total := m_income - t_sal;
			--return profit;
		end if;
		return total;
	...
	begin
		...
		if(show_f = 0) then
			for i in (select g_name,g_age,g_gender,g_height,g_sal, g_experience,g_rating from guards 
						...
						AND (lower(g_speciality) = lower(spc)) AND (g_status = 'Free') AND (g_branch = 'Dhaka'))loop
			...
			for i in (select g_name,g_age,g_gender,g_height,g_sal, g_experience,g_rating from guards 
						...
						AND (g_status = 'Free') AND (g_branch = 'Dhaka'))loop
				DBMS_OUTPUT.PUT_LINE('Guard  '|| j );
				j := j+1;
			...
			for i in (select g_name,g_age,g_gender,g_height,g_sal, g_experience,g_rating from guards 
						where (lower(g_speciality) = lower(spc)) AND (g_status = 'Free') AND (g_branch = 'Dhaka'))loop
				DBMS_OUTPUT.PUT_LINE('Guard  '|| j );
				j := j+1;
	...
	BEGIN
		...
		select SUM(payment_amount),count(*) into m_total ,rowno from payment where payment_date LIKE '%January%2020' 
					AND cid IN(select cid from clients where c_branch = 'Dhaka');
		if(rowno = 0)then
			m_total :=0;
		end if;
	...
	begin
		...
		for i in (select g_name,g_age,g_gender,g_height,g_sal, g_experience,g_rating,g_speciality from guards 
					where g_status = 'Free' AND g_branch = 'Dhaka')loop
			DBMS_OUTPUT.PUT_LINE('Guard  '|| j );
			j := j+1;
	...
	DBMS_OUTPUT.PUT_LINE('Printing new line');
