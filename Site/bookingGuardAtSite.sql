...
BEGIN
	...
	select gid into g_no from guards where g_name = name AND g_age = age;
	select g_status into sts from guards where gid = g_no;
	if(sts = 'Busy')then
	 raise not_available;
	end if;
	select count(*) into cnt from clients where c_name = client_name AND c_mail = email;
	
	if(cnt > 0)then
		select cid into c_no from clients where c_name = client_name AND c_mail = email;
		myPackage.makePayment(g_no,c_no);
	end if;
	if(cnt = 0 )then
		DBMS_OUTPUT.PUT_LINE('New client');
		insert into clients values(c_no,client_name,'Dhaka',email,'Service','Chittagong');
		commit;
		myPackage.makePayment(g_no,c_no);
		c_no := c_no + 1 ;
	end if;
